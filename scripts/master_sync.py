...
def main():
    print(f"\n[MASTER] Institutional Data Pipeline initialized at {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    
    # 1. Market Sync (Fetch prices and setup DB)
    if not run_script("market_sync.py"):
        print("[MASTER] Critical Failure in Market Sync. Aborting pipeline.")
        return

    # 2. AI Analyzer (Sentiment Analysis)
    run_script("ai_analyzer.py")

    # 3. Alpha Scanner (Technical Signals)
